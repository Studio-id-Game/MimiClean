...
namespace StudioIdGames.MimiClean.Railway
{
    ...
    public readonly struct CleanResultStruct<TResult> : ICleanResult<TResult>
    {
        ...
        /// <summary>
        /// <typeparamref name="TResult"/> への暗黙的変換を許可します。ただし、 <see cref="State"/> が <see cref="CleanResultState.Success"/> 以外の時は <see cref="InvalidCastException"/> をスローします。
        /// </summary>
        /// <param name="t"></param>
        /// <returns></returns>
        public static implicit operator TResult(in CleanResultStruct<TResult> t)
        {
            return t.State == CleanResultState.Success ? t.Result :
                throw new InvalidCastException($"Can't implcit cast when {nameof(Result)} is not {nameof(CleanResultState.Success)}");
        }
